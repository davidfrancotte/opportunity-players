...
import { members, isPremium } from "@/lib/studio/social";
...
export function PlayPage() {
  ...
  return (
    <ProfileLayout>
      ...
      <nav className="extension-nav"><Link href="/espace/recherches">Alertes de matchs</Link><Link href="/espace/calendrier-avance">Récurrence & agenda</Link></nav>
...
export function CreateMatchPage() {
  ...
  useEffect(() => {
    if (submitted && events.lastCreated === submitted) router.push(`/espace/match?id=${submitted}`);
...
export function AgendaPage() {
  ...
  return (
    <ProfileLayout>
      ...
      {entries.map(({ m, slot }) => (
        <Link className="agenda-item" key={m.id + slot.id} href={`/espace/match?id=${m.id}`}>
