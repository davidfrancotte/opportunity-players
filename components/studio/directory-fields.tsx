  ...
  return (
    <div className="directory-fields">
      ...
      <Field
        ...
        placeholder={uiCopy("Choisir ou préciser un poste")}
      ...
      {profile.category === "Sportif" ? (
        <>
          ...
          <Field
            ...
            maxLength={80}
            placeholder={uiCopy("Ex. P200, C15.2, division…")}
          />
          <p className="field-hint">
            <T>{"Niveau, poste et côté dominant sont propres à chaque sport. Complétez vos autres disciplines depuis Profil → Sports, niveaux &amp; clubs."}</T></p>
        </>
      ) : (
        <Field label="Secteur / métier" name="accountType" error={errors.accountType}>
          <NativeSelect
            id="accountType"
            aria-label={uiCopy("Secteur / métier")}
            required
          ...
          >
            <NativeSelectOption value="">
              <T>{"Choisir"}</T>{" "}
              <T>{profile.category === "Professionnel" ? "votre métier" : "le type de structure"}</T>
            </NativeSelectOption>
