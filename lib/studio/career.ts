  ...
  return [
    {
      ...
      sports: [
        ...new Set(
          [profile.sport, ...profile.disciplines.map((d) => d.sport)].filter(
            Boolean,
          ),
        ),
      ],
    ...
    ...members.map((m) => ({
      ...
      sports: [
        ...new Set([
          m.sport,
          ...(memberSports[m.id] || []).map((d) => d.sport),
        ]),
      ],
      city: m.city,
      country: m.country,
      ...
      dossier: [
        ...
        ...(memberSports[m.id] || []).map((d) =>
          [d.sport, d.level, d.position, d.availability]
            .filter(Boolean)
            .join(' · '),
        ),
      ],
  ...
  return {
    offers: [
      ...
      },
      {
        ...
        position: 'Ailier',
        description:
          'Détection amateur. Postes extérieurs et intérieurs. Aucune sélection réelle.',
        open: true,
      },
      {
        id: 'padel-trial',
...
const clean = (s: string, max = 160) =>
  !!s.trim() && s.length <= max && !moderateText(s);
const future = (s: string, now: number) =>
  Number.isFinite(Date.parse(s)) && Date.parse(s) > now;
const overlap = (a: string, b: string) =>
  Math.abs(Date.parse(a) - Date.parse(b)) < 30 * 60000;
...
export function careerReducer(
  ...
  if (action.type === 'preferences')
    return {
      ...base,
      preference: {
        sport: action.sport.slice(0, 80),
        city: action.city.trim().slice(0, 80),
      },
    };
  if (action.type === 'dismiss')
    return {
      ...base,
      dismissed: [...new Set([...state.dismissed, action.id])],
    };
  if (action.type === 'read')
    return {
      ...base,
      notices: state.notices.map((n) =>
        n.recipient === actor.id && (!action.id || n.id === action.id)
          ? { ...n, read: true }
          : n,
      ),
    };
  ...
  if (action.type === 'close-offer') {
    if (
      !recruiter ||
      !state.offers.some((o) => o.id === action.id && o.owner === actor.id)
    )
      return fail('forbidden');
    return {
      ...base,
      offers: state.offers.map((o) =>
        o.id === action.id ? { ...o, open: false } : o,
      ),
    };
  }
  if (action.type === 'apply') {
    ...
    if (
      state.applications.some(
        (a) =>
          a.id === action.id ||
          (a.offerId === o.id && a.candidate === actor.id),
      )
    )
  ...
  if (action.type === 'stage') {
    ...
    if (!a || !o || !allowed(a.candidate) || !allowed(o.owner))
      return fail('forbidden');
    ...
    return notify(
      ...
      text[0] + ' : ' + o.title,
      text[1] + ': ' + o.title,
      own ? '/espace/recrutement' : '/espace/candidatures',
    );
  }
    ...
    return notify(
      {
        ...
        appointments: state.appointments.map((x) =>
          x.id === a.id
            ? { ...x, status: action.accept ? 'accepted' : 'declined' }
            : x,
        ),
      },
      a.requester,
      ...
      action.accept
        ? 'Request accepted: choose a time'
  ...
  if (action.type === 'slot') {
    ...
    if (
      state.slots.some(
        (x) =>
          x.id === s.id ||
          (x.professional === actor.id && overlap(x.start, s.start)),
      )
    )
      return fail('overlap');
    return {
      ...base,
      slots: [...state.slots, s].sort(
  ...
  if (action.type === 'remove-slot') {
    if (
      !state.slots.some(
        (s) => s.id === action.id && s.professional === actor.id,
      ) ||
      state.appointments.some(
        (a) => a.slot === action.id && a.status === 'booked',
      )
    )
      return fail('forbidden');
    ...
    if (
      state.appointments.some(
        (x) =>
          ...
          x.status === 'booked' &&
          state.slots.some(
            (s) => s.id === x.slot && overlap(s.start, chosen.start),
          ),
      )
    )
      return fail('overlap');
...
export function recommendations(
  ...
  const wantedSports = state.preference.sport
    ? [state.preference.sport]
    : actor.sports;
  ...
  function reasons(sports: string[], place: string, country: string) {
    ...
    if (sports.some((s) => wantedSports.map(norm).includes(norm(s))))
      result.push('sport');
    if (city && norm(city) === norm(place)) result.push('city');
    else if (actor.country && norm(actor.country) === norm(country))
      result.push('country');
    return result;
  }
  const rank = (r: string[]) =>
  ...
  return {
    ...
    offers: state.offers
      ...
      .map((o) => ({
        offer: o,
        reasons: reasons([o.sport], o.city, o.country),
      }))
      .filter((x) => x.reasons.length)
      .sort((a, b) => rank(b.reasons) - rank(a.reasons))
      .slice(0, 2),
