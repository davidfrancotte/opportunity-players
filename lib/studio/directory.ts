...
export function sideLabel(sport: string) {
  return ['Football', 'Futsal'].includes(sport)
    ? 'Pied dominant'
    : 'Bras / pied dominant';
}
...
export function updatePrimaryRecord(
  profile: Profile,
  patch: Partial<SportRecord>,
): Profile {
...
export function changeDirectoryKind(
...
export function matchesDirectory(
  ...
  if (!contains(member.country, f.country) || !contains(member.city, f.city))
    return false;
  ...
  if (playerFilters && f.gender !== 'Tous' && member.gender !== f.gender)
    return false;
  if (
        ...
        (r) =>
          ...
          (!playerFilters ||
            ((!f.paraSport ||
              ...
              (f.dominantSide === 'Tous' ||
                r.dominantSide === f.dominantSide) &&
              contains(r.ranking, f.ranking) &&
              (!f.club.trim() ||
                r.clubs.some((c) => contains(c.name, f.club))))),
      )
