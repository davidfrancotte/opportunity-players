...
export function sportProfileIssues(
  p: Pick<
    Profile,
    'birthDate' | 'registrationMode' | 'guardian' | 'category' | 'disciplines'
  >,
) {
  const errors: Record<string, string> = {};
  const age = ageOn(p.birthDate);
  if (p.birthDate && age === null)
    errors.birthDate = 'Indiquez une date de naissance valide.';
  if (p.registrationMode === 'child') {
    if (p.category !== 'Sportif')
      errors.category = 'Le profil enfant est un profil sportif.';
    if (age === null || age >= 18)
      errors.birthDate =
        'Le profil enfant nécessite une date de naissance et un âge inférieur à 18 ans.';
    if (!p.guardian?.name.trim())
      errors.guardianName = 'Indiquez le nom du représentant.';
    ...
    if (!p.guardian?.consent)
      errors.guardianConsent =
        'La confirmation du représentant est nécessaire.';
...
export function videoFileIssue(file: { type: string; size: number }) {
  if (!['video/mp4', 'video/webm'].includes(file.type))
    return 'Choisissez une vidéo MP4 ou WebM.';
