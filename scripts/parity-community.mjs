...
const nav = async (href) => {
  await page.locator(`a[href="${href}"]`).filter({visible:true}).first().press("Enter");
  await page.waitForURL(`**${href}`);
};
try {
  await page.goto("http://127.0.0.1:3000/espace/accueil", { waitUntil: "networkidle" });
  ...
  assert.equal(
    ...
    "/espace/abonnement?retour=/espace/accueil",
  );
  await page
    .locator(".community-schedule")
    .screenshot({ path: "/tmp/web-parity-schedule-free.png", animations: "disabled" });
  await page.getByLabel("Fermer la fenêtre").click();
  await page.screenshot({ path: "/tmp/web-parity-community-free-home.png", fullPage: true });
  ...
  assert.equal(
    await page.getByRole("link", { name: "Découvrir Premium" }).getAttribute("href"),
    "/espace/abonnement?retour=/espace/reseau",
  );
  for (const label of [
    "Filtrer par pays",
    "Filtrer par ville",
  ...
  await page
    ...
    .first()
    .screenshot({ path: "/tmp/web-parity-network-free-filters.png", animations: "disabled" });
  await refine.click();
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-free-network.png", fullPage: true });
  ...
  await page
    ...
    .setInputFiles("public/studio-images/tennis-color.webp");
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-schedule.png", fullPage: true });
  await page.getByRole("button", { name: "Programmer", exact: true }).click();
  await page.getByRole("dialog").waitFor({ state: "hidden" });
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-premium-home.png", fullPage: true });
  await nav("/espace/reseau");
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-premium-network.png", fullPage: true });
  await page.locator('.network-sections a[href="/espace/jouer"]').click();
  await page.getByRole("button", { name: "À proximité", exact: true }).click();
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-nearby.png", fullPage: true });
  ...
  await page.screenshot({ path: "/tmp/web-parity-community-test-failure.png", fullPage: true });
  throw error;
} finally {
  await browser.close();
}
