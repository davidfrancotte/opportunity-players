...
const photo = path.resolve("public/studio-images/padel-color.webp");
try {
  await page.goto("http://127.0.0.1:3000/espace/medias", { waitUntil: "networkidle" });
  await open();
  ...
  await close();
  await page.locator('.bottom-nav a[href="/espace/profil"]').click();
  await page.locator('a[href="/espace/medias"]').first().click();
  ...
  await choose(photo);
  await page.getByAltText("Aperçu du média sélectionné").waitFor();
  await page.screenshot({ path: "/tmp/web-parity-media-upload-preview.png", animations: "disabled" });
  await close();
  assert.equal(await page.locator(".media-grid img").count(), 2);
