...
test('inscription : identité et adresse valides, mot de passe exclusivement fictif', () => {
  ...
  assert.ok(
    validateIdentity({ ...identity, firstName: ' ' }, DEMO_PASSWORD).firstName,
  );
  assert.ok(
    validateIdentity({ ...identity, lastName: ' ' }, DEMO_PASSWORD).lastName,
  );
  assert.ok(
    validateIdentity({ ...identity, email: 'faux' }, DEMO_PASSWORD).email,
  );
...
test('connexion explicitement fictive : identifiants connus ou profil de la visite', () => {
  assert.deepEqual(
    validateDemoLogin(DEMO_EMAIL, DEMO_PASSWORD, 'new@demo.example'),
    {},
  );
  assert.deepEqual(
    validateDemoLogin('NEW@demo.example', DEMO_PASSWORD, 'new@demo.example'),
    {},
  );
  assert.ok(
    validateDemoLogin('unknown@demo.example', DEMO_PASSWORD, DEMO_EMAIL).email,
  );
...
test('profil : champs requis et parcours organisation', () => {
  ...
  assert.ok(
    validateProfile({ ...initialProfile, sport: 'sport-inconnu' }).sport,
  );
...
test('CV téléchargé : données courantes, avertissement fictif, aucun email ou secret', () => {
  ...
  assert.ok(
    cvText({ ...initialProfile, firstName: 'Camille' }).includes(
      'Camille Dupont',
    ),
  );
});
test('frontière démo : aucun stockage navigateur ni envoi réseau applicatif', () => {
  for (const name of [
    ...
    const source = fs.readFileSync(
      new URL('../components/studio/' + name, import.meta.url),
      'utf8',
    );
    assert.doesNotMatch(
      source,
  ...
  const provider = fs.readFileSync(
    new URL('../components/studio/demo-provider.tsx', import.meta.url),
    'utf8',
  );
