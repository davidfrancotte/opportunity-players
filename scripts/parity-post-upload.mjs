...
try {
  ...
  await (await chooser).setFiles("public/studio-images/padel-color.webp");
  ...
  await choose("public/studio-images/padel-color.webp");
  await page.getByAltText("Aperçu de la photo à publier").waitFor();
  ...
  await page
    .getByRole("dialog")
    .screenshot({ path: "/tmp/web-parity-post-video-upload.png", animations: "disabled" });
  await publish();
  ...
  assert.equal(await page.locator(".post-attachment-preview").count(), 0);
  await close();
  await page.goto("http://127.0.0.1:3000/espace/abonnement?retour=/espace/accueil");
  ...
  await page.getByRole("dialog").waitFor({ state: "hidden" });
  await page.locator('.bottom-nav a[href="/espace/accueil"]').press("Enter");
  await open();
  await choose(video);
  await page.getByLabel("Aperçu de la vidéo à publier").waitFor();
