...
import {
  ...
  sportProfileIssues,
  videoFileIssue,
} from '../lib/studio/sport-profile.ts';
import { initialProfile, cvText, createProfile } from '../lib/studio/model.ts';
import {
  matchesDirectory,
  emptyDirectoryFilters,
  changeDirectoryKind,
} from '../lib/studio/directory.ts';
import { members } from '../lib/studio/social.ts';
import { memberSports } from '../lib/studio/trust.ts';
...
test('child route: guardian, consent, minority and athlete category are mandatory', () => {
  ...
  assert.ok(
    sportProfileIssues({
      ...child,
      guardian: { ...child.guardian, consent: false },
    }).guardianConsent,
  );
  assert.ok(
    sportProfileIssues({ ...child, category: 'Professionnel' }).category,
  );
  assert.ok(
    sportProfileIssues({ ...child, birthDate: '1990-01-01' }).birthDate,
  );
  assert.ok(
    sportProfileIssues({ ...child, registrationMode: 'self' }).birthDate,
  );
...
test('network: age and para-sport combine with sport, availability and contract', () => {
  ...
  assert.equal(
    matchesDirectory({ ...lea, birthDate: undefined }, records, f),
    false,
  );
...
test('sports video: MIME and size bounds', () => {
  ...
  assert.equal(
    videoFileIssue({ type: 'video/webm', size: 50 * 1024 * 1024 }),
    '',
  );
  for (const file of [
...
test('export includes achievements but not private date, licence, guardian or video object URL', () => {
  const p = {
    ...
    disciplines: [
      {
        ...initialProfile.disciplines[0],
        licenceNumber: 'SECRET-12345',
        paraSport: 'yes',
      },
    ],
    achievements: [
