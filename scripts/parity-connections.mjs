...
async function nav(href) { await page.locator(`a[href="${href}"]`).filter({visible:true}).first().press('Enter'); await page.waitForURL(`**${href}`); }
...
try {
  ...
  for (const width of [320, 390, 480, 900]) {
    ...
    if (width === 390) await actions.screenshot({ path: '/tmp/web-parity-member-actions.png', animations: 'disabled' });
  }
  await page.setViewportSize({ width: 390, height: 844 });
  await close(); await nav('/espace/accueil');
  ...
  await dialog.screenshot({ path: '/tmp/web-parity-connected-profile.png', animations: 'disabled' });
