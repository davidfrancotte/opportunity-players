...
try {
  await page.goto("http://127.0.0.1:3000/espace/jouer", { waitUntil: "networkidle" });
  assert.equal(await page.locator(".network-sections a").count(), 3);
  await page.locator('.network-sections a[href="/espace/agenda"]').click();
  ...
  await page.screenshot({ path: "/tmp/web-parity-agenda-mobile.png", fullPage: true });
