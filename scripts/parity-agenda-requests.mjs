...
async function request() {
  await page.locator('.bottom-nav a[href="/espace/reseau"]').click();
...
async function agenda() {
  await page.locator('.career-nav a[href="/espace/agenda"]').click();
  await page.waitForURL("**/agenda");
  await page.locator(".agenda-requests-heading").click();
}
try {
  await page.goto("http://127.0.0.1:3000/espace/rendez-vous", { waitUntil: "networkidle" });
  await request();
  await actor("marc");
  await agenda();
  ...
  await page
    ...
    .screenshot({ path: "/tmp/web-parity-agenda-requests.png", animations: "disabled" });
